module.exports = {
  ...
  GSP: function (data, socket) {
    ...
    if (typeof data.sno == "undefined") {
      commonClass.SendData(
        { Message: "Guestsignup sno is not defined", en: data.en, sc: 0 },
        socket.id
      );
      return false;
    }
    if (typeof data.det == "undefined") {
      commonClass.SendData(
        { Message: "Guestsignup det is not defined", en: data.en, sc: 0 },
        socket.id
      );
      return false;
    }
    if (typeof data.ult == "undefined") {
      commonClass.SendData(
        { Message: "Guestsignup ult is not defined", en: data.en, sc: 0 },
        socket.id
      );
      return false;
    }
        ...
        function (err, resp) {
          if (resp) {
            ...
            signupClass.getDefaultFields(data, socket, function (tData) {
              ...
              userData.userID =
                "G00" + Math.floor(Math.random() * 90000000 + 10000000);
              signupClass.RefLink(function (RefLink) {
                userData.reflink = RefLink;
                db.collection("game_users").insertOne(
                  userData,
  ...
  TPSP: function (data, socket) {
    ...
    if (typeof data.data == "undefined") {
      commonClass.SendData(
        {
          Message: "FBSignup request is not in data object",
          en: data.en,
          sc: 0,
        },
        socket.id
    ...
    if (typeof data.det == "undefined") {
      commonClass.SendData(
        { Message: " det is not define", en: data.en, sc: 0 },
        socket.id
      );
      return false;
    }
      ...
      userSettingCases.getLuckyCard(function (err, luckyCard) {
        signupClass.thirdpartyLoginOrSignup(data, socket, function (err, res) {
          ...
          if (roomId) {
            ...
            commonClass.SendData(
              { data: res, IR: false, en: data.en, lc: luckyCard.INFO, sc: 1 },
              socket.id
            );
          }
        });
    ...
    db.collection("game_users").findOne({ tId: data.tId }, function (err, res) {
      signupClass.getDefaultFields(data, socket, function (userData) {
        if (!res) {
          ...
          fuserData.userID =
            "TP00" + Math.floor(Math.random() * 90000000 + 10000000);
          ...
          if (res.socketId != "") {
            commonClass.SendData(
              {
                msg: "You are logging in another device.",
                en: "logout",
                sc: 1,
              },
              res.socketId
  ...
  RefLink: function (callback) {
    ...
    Bitly.shorten(
      { longUrl: config.BASE_URL + ":" + config.SERVER_PORT + "/" + uniqid() },
