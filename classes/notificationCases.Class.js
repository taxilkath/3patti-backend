module.exports = {
	...
	SM:function(data, socket){
		...
		var otherId = objectId(other_userid[0]); 
			...
			if(userdata){
				...
				db.collection('game_users').findOneAndUpdate({uid: userdata.uid},{$inc:{"counters.urmsg":1}},{returnOriginal:false}, function(err, user_data){
					if(err){
		 			...
		 			if(user_data.value && user_data.value.socketId != ''){
				...
				db.collection('game_users').findOne({_id: objectId(data.user_id)},function(err, user_data){
					...
					if(user_data){
						...
						if( user_chips > chips ){
							...
							db.collection('game_chips_tracker').insertOne({ chips : chips,type : 'debit', msg : 'RSC', price: result.price ,"ct" : "chips", uid : user_id, orderId:data.orderId});
