module.exports = function () {
  ...
  app.get("/", function (req, res, next) {
    res.json({
      status_code: 1,
      message: "hooray! welcome to our Node.js rest api with MongoDB database!",
    });
  });
  ...
  app.options("/chooseServer/:DeviceType/:v", function (req, res, next) {
    ...
    db.collection("server_info").findOne(function (err, sinfo) {
      if (!err) {
        res.json({
          status_code: 1,
          info: sinfo,
          fc: config.NEWUSER_FBCHIPS,
          rvchips: config.REWARD_VIDEO_CHIPS,
          variation: config.GAME_VARIATIONS,
        });
      } else {
        res.json({
          status_code: 0,
  ...
  //find and return free server for UNITY DEVELOPMENT
  ...
  /* GET home page. */
  app.get("/upp", function (req, res, next) {
    if (!req.files) return res.status(400).send("No files were uploaded.");

    if (!req.body.user_id) return res.status(400).send("userid missing");
  ...
  app.get("/:id", function (req, res, next) {
    ...
    if (id) {
      ...
      if (md.os() == "AndroidOS") {
        printLog("AndroidOS");
        res.redirect(
          "https://play.google.com/store/apps/details?id=com.indian.teen.patti.flush.king&referrer=" +
            id
        );
      } else {
        printLog("other");
  ...
  app.get("/maintenance", function (req, res, next) {
    db.collection("server_info").findOne(function (err, sinfo) {
      if (!err) {
        ...
        db.collection("server_info").updateOne({ _id: sinfo._id }, upWhere);
